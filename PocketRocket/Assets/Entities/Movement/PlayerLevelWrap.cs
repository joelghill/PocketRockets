...
namespace PocketRockets.Entities.Movement
{
    public class PlayerLevelWrap : MonoBehaviour
    {
        ...
        void Start()
        {
            this.VesselRigidBody = this.Player.GetComponent<Rigidbody2D>();
            this.levelBoundingBox = Level.GetComponent<LevelBoundingBox2D>();
            this.rocket2DBounds = this.Player.GetComponent<ModularRocket2DBounds>();
        }
        ...
        void Update()
        {
            if (this.rocket2DBounds != null)
            {
                ...
                {
                    var oldPos = this.Player.transform.position;
                    this.Player.transform.position = new Vector3(this.levelBoundingBox.GetLeftEdge() + (vesselBoundingSize.x / 2), oldPos.y, oldPos.z);
                }

                if (targetBounds.Right < this.levelBoundingBox.GetLeftEdge() && VesselRigidBody.velocity.x < 0)
                {
                    var oldPos = this.Player.transform.position;
                    this.Player.transform.position = new Vector3(this.levelBoundingBox.GetRightEdge() - (vesselBoundingSize.x / 2), oldPos.y, oldPos.z);
                }
            }
        }
